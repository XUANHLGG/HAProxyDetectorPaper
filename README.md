...
A lightweight plugin for Paper/Purpur and Velocity that makes native HAProxy / Proxy Protocol support compatible with normal direct player connections.
...
- **Minimal Intercept Logging**: Logs only when a non-whitelisted proxy connection is blocked, including the frps source IP and client IP.
...
- **Paper / Purpur**: Theoretically supports versions with native HAProxy support, tested on `1.20.1 - 1.21.4`.
...
```
[HAProxyDetectorPaper] Blocked non-whitelisted frps connection: frps=91.78.69.13, client=91.78.69.13
```

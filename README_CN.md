...
一个轻量级插件，用于在 Paper/Purpur 和 Velocity 上同时兼容原生 HAProxy / Proxy Protocol 与普通玩家直连。
...
- **Paper / Purpur**：理论支持带原生 HAProxy 功能的版本，已在 `1.20.1 - 1.21.4` 测试。
- **Velocity**：理论支持 `3.0.0+`。
...
1. **开启服务端原生 HAProxy 支持**：
   - **Paper / Purpur**：在 `config/paper-global.yml`（新版本）或 `paper.yml`（旧版本）中将 `proxy-protocol` 设为 `true`。
